...
class TestMatchKnownError:
    ...
    def test_evidence_strength_without_connector(self):
        """Só texto digitado → evidence_strength reduzido (0.70)."""
        result = match_known_error("oauth token expired", has_connector_data=False)
        assert result is not None
        assert result["evidence_strength"] == 0.70

    def test_evidence_strength_with_connector(self):
        """Conector real presente → evidence_strength máximo (0.95)."""
        result = match_known_error("oauth token expired", has_connector_data=True)
        assert result is not None
        assert result["evidence_strength"] == 0.95
